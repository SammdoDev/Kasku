  ...
  return (
    ...
    >
        ...
        {!lockType && (
          <div className="flex flex-shrink-0">
            {(["line", "bar"] as ChartType[]).map((key, i) => (
              ...
              >
                {key === "line" ? CONSTANT.chartLine : CONSTANT.chartBar}
              </button>
            ))}
          </div>
        )}
      ...
      <div className="relative w-full" style={{ height }}>
        {isEmpty ? (
          <div className="w-full h-full flex flex-col items-center justify-center gap-1.5 border-[2.5px] border-dashed border-[#ccc]">
            <span className="text-[10px] font-bold tracking-[2px] uppercase text-[#bbb]">
              {CONSTANT.noData}
            </span>
            <span className="text-[8px] text-[#ddd]">
              {emptyMessage ?? CONSTANT.noDataPeriod}
            </span>
          </div>
        ) : (
